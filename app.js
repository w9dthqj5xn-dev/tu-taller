...
function filtrarOrdenes() {
    ...
    ordenes.forEach(orden => {
        ...
        html += `<div class="orden-card"><div class="orden-header"><span class="orden-numero">Orden #${orden.numero}</span><span class="badge badge-${getEstadoClass(orden.estado)}">${orden.estado}</span></div><div class="orden-info"><div class="info-item"><span class="info-label">Cliente:</span> ${clienteNombre}</div><div class="info-item"><span class="info-label">Dispositivo:</span> ${orden.marca} ${orden.modelo}</div><div class="info-item"><span class="info-label">Tipo:</span> ${orden.tipoDispositivo}</div><div class="info-item"><span class="info-label">Fecha Ingreso:</span> ${new Date(orden.fechaIngreso).toLocaleDateString()}</div>${orden.presupuesto ? `<div class="info-item"><span class="info-label">Presupuesto:</span> $${orden.presupuesto.toFixed(2)}</div>${orden.costoPiezas > 0 ? `<div class="info-item"><span class="info-label">Costo Piezas:</span> $${orden.costoPiezas.toFixed(2)}</div>` : ''}<div class="info-item"><span class="info-label">Anticipo:</span> $${orden.anticipo.toFixed(2)}</div><div class="info-item"><span class="info-label">Saldo:</span> <strong>$${saldo.toFixed(2)}</strong></div>` : ''}${orden.tecnico ? `<div class="info-item"><span class="info-label">Técnico:</span> ${orden.tecnico}</div>` : ''}</div><p><strong>Problema:</strong> ${orden.problema}</p>${orden.notas ? `<p><strong>Notas:</strong> ${orden.notas}</p>` : ''}${repuestosHtml}${garantiaHtml}<div style="margin-top: 15px;"><button class="btn-success" onclick="editarOrden(${orden.id})">Editar</button><button class="btn-secondary" onclick="cambiarEstadoOrden(${orden.id})">Cambiar Estado</button><button class="btn-primary" onclick="imprimirRecibo(${orden.id})">📄 Imprimir</button><button class="btn-primary" onclick="enviarWhatsApp(${orden.id})" style="background: #25d366;">📱 WhatsApp</button>${saldo > 0 ? `<button class="btn-primary" onclick="registrarPago(${orden.id})">💰 Pagar</button>` : ''}<button class="btn-danger" onclick="eliminarOrden(${orden.id})">Eliminar</button></div></div>`;
    });
    container.innerHTML = html;
}
...
function generarReportes() {
    ...
    let gananciasManoDeObra = 0;
    let gananciasPiezas = 0;
    let gananciasRepuestos = 0;
    ...
    ordenesFiltradas.forEach(orden => {
        ...
        // Mano de obra = Presupuesto - (Costo Piezas + Costo Repuestos)
        const manoDeObra = presupuesto - costoPiezas - costoRepuestos;
        
        gananciasManoDeObra += manoDeObra;
        gananciasPiezas += costoPiezas;
        gananciasRepuestos += costoRepuestos;
        anticiposTotales += orden.anticipo || 0;
    });
    
    const porcentajeManoObra = ingresosTotales > 0 ? (gananciasManoDeObra / ingresosTotales * 100) : 0;
    const porcentajePiezas = ingresosTotales > 0 ? (gananciasPiezas / ingresosTotales * 100) : 0;
    const porcentajeRepuestos = ingresosTotales > 0 ? (gananciasRepuestos / ingresosTotales * 100) : 0;
    
    let htmlGanancias = '<table><thead><tr><th>Tipo de Ingreso</th><th>Monto</th><th>Porcentaje</th></tr></thead><tbody>';
    htmlGanancias += `<tr style="background: #e3f2fd;"><td><strong>💼 Mano de Obra</strong></td><td><strong>$${gananciasManoDeObra.toFixed(2)}</strong></td><td>${porcentajeManoObra.toFixed(1)}%</td></tr>`;
    htmlGanancias += `<tr style="background: #fff3e0;"><td><strong>🔧 Piezas Externas</strong></td><td><strong>$${gananciasPiezas.toFixed(2)}</strong></td><td>${porcentajePiezas.toFixed(1)}%</td></tr>`;
    htmlGanancias += `<tr style="background: #f3e5f5;"><td><strong>📦 Repuestos Inventario</strong></td><td><strong>$${gananciasRepuestos.toFixed(2)}</strong></td><td>${porcentajeRepuestos.toFixed(1)}%</td></tr>`;
    htmlGanancias += `<tr style="background: #c8e6c9; font-weight: bold;"><td><strong>💰 TOTAL INGRESOS</strong></td><td><strong>$${ingresosTotales.toFixed(2)}</strong></td><td>100%</td></tr>`;
